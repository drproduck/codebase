...
class PartialIwaeXY(nn.Module):
    ...
    def __init__(self, X_DIM, Y_DIM, Z_DIM, V_DIM,
        ...
        self.Z_DIM = Z_DIM
        self.V_DIM = V_DIM
        self.x_encoder_xy = nets.LinearMap(X_DIM, [500], 50)
        self.x_encoder_xz = nets.LinearMap(X_DIM, [500], 50)
        self.y_encoder_xy = nets.LinearMap(Y_DIM, [500], 50)
        self.z_encoder_xz = nets.LinearMap(Z_DIM, [500], 50)
        self.z_encoder_zv = nets.LinearMap(Z_DIM, [500], 50)
        self.v_encoder_zv = nets.LinearMap(V_DIM, [500], 50)

        # inference networks
        xy_reduced_dim = self.x_encoder_xy.get_output_dim() + self.y_encoder_xy.get_output_dim()
        self.z_given_xy = nets.GaussianStochasticEncoder(xy_reduced_dim, Z_DIM, hidden_dims=[500, 500], hidden_acts=nn.ReLU())

        xz_reduced_dim = self.x_encoder_xz.get_output_dim() + self.z_encoder_xz.get_output_dim()
        ...
        zv_reduced_dim = self.z_encoder_zv.get_output_dim() + self.v_encoder_zv.get_output_dim()
    ...
    def forward(self,
        ...
        x_reduced_xy = self.x_encoder_xy(x_outer)
        y_reduced_xy = self.y_encoder_xy(y_outer)
        xy_reduced = torch.cat((x_reduced_xy, y_reduced_xy), dim=-1)
        z_outer, _, _, log_posterior_z_given_xy = self.z_given_xy(xy_reduced)
        ...
        y_inner = repeat_newdim(y_outer, n_samples_v, -2)
        ...
        x_reduced_xz = self.x_encoder_xz(x_inner)
        z_reduced_xz = self.z_encoder_xz(z_inner)
        xz_reduced = torch.cat((x_reduced_xz, z_reduced_xz), dim=-1)
        ...
        v_reduced_zv = self.v_encoder_zv(v_inner)        
        zv_reduced = torch.cat((z_reduced_zv, v_reduced_zv), dim=-1)
        x_recon = self.x_given_zv(zv_reduced)
        y_recon = self.y_given_z(z_inner)

        img_log_likelihood_x = log_prob_bernoulli(x_recon, x_inner)
        img_log_likelihood_y = log_prob_bernoulli(y_recon, y_inner)
        ...
        inner_lowerbound = img_log_likelihood_x + img_log_likelihood_y + log_prior_v - log_posterior_v_given_xz
        ...
        outer_lowerbound = inner_lowerbound + log_prior_z - log_posterior_z_given_xy
